  ...
  def __init__(self):
    ...
    html = '''
      ...
      <div class='methods'>
        ...
        <p><b>2. Missing percentage (%)</b>: Too many missing values interfere with comparability. This metric is the median of missing percentages among samples.</p>
        ...
        <p><b>6. Relative Correlation with Reference Datasets (RC)</b>: RC is used for assessment of quantitative consistency with the reference dataset at relative levels. For shotgun proteomics, quantitation at peptide levels is theoretically more reliable. Therefore, the reference dataset is established by benchmarking the relative expression values, for each peptide sequence of each sample pair (D5/D6, F7/D6, M8/D6), in historical datasets at peptide levels. We calculate relatively qualified (satisfied with thresholds of p < 0.05) expression values of the queried data, for overlapped peptides with the reference dataset, as the input for the assessment of quantitative consistency. Then RC value is Pearson correlation coefficient between the test dataset and the reference dataset.</p>
        </div>
      </div>
